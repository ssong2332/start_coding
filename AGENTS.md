...
(다이어그램은 요약이다 — 게이트 전체와 복귀 경로는 아래 표·불릿이 기준. 아이디어 인터뷰 시작 시 `.agents/skills/idea-interview/SKILL.md`의 "진행 방식 추천" 단계를 거쳐 사용자가 vibe 모드를 확정하거나, 진행 중 사용자가 명시적으로 우회를 요청하면 `.agents/skills/vibe-mode/SKILL.md`의 대안 경로를 쓴다 — 아래 표·게이트는 그 경로에 적용되지 않는다.)
...
- **날짜 전달 의무: 문서에 날짜를 적어야 하는 작업(PRD·Architecture·DECISIONS·adr·CHANGELOG 등)을 서브에이전트에 맡길 때는 호출 프롬프트에 오늘 날짜를 명시한다.** 서브에이전트는 실행 환경의 실제 날짜를 모른다 — 명시하지 않으면 추측해 틀린 날짜를 문서에 남길 수 있다.
...
| 진행 중 사용자가 기능 추가·요구사항 변경 요청 | planner — 아래 "진행 중 요구사항 변경" 표에 따라 architect 추가 호출 여부 결정 |

## 진행 중 요구사항 변경

구현이 시작된 뒤에도 사용자는 기능을 추가하거나 기존 요구사항을 바꿀 수 있다 — PRD·Architecture 승인은 그 시점까지 정한 것을 고정할 뿐, 이후 변경을 막는 장치가 아니다. 애자일에서 가져올 값은 이것 하나다: 변경에 대응하는 것. 스프린트·백로그 같은 의례는 가져오지 않는다 — 이미 vibe 모드가 그 쪽을 담당한다. 변경 요청은 이 표로 분류해 처리한다:

| 변경 성격 | 처리 | 재승인 범위 |
|---|---|---|
| 기존 아키텍처·데이터 모델·인터페이스 규격을 안 건드리는 새 기능/사소한 수정 | planner가 PRD에 F-xx 추가(또는 기존 F-xx를 그 자리에서 수정) + Tasks.md에 새 T-xx 추가 | 추가·수정된 요구사항만 사용자 확인 — PRD 전체 재승인 불필요, 머리글 "상태"도 "승인"으로 그대로 둔다 |
| 기존 아키텍처·인터페이스 규격을 바꿔야 하는 변경 | planner가 PRD 갱신 → architect가 Architecture·DECISIONS·필요하면 새 ADR을 갱신(승인된 기존 ADR은 수정하지 않고 새 ADR로 대체 — 기존 규칙과 동일) | 바뀐 부분만 사용자 확인 |
| 이미 "완료"된 태스크의 구현을 바꿔야 함 | 완료 상태·근거는 그대로 두고, 새 T-xx(재작업 태스크)를 만들어 리뷰·QA를 다시 거친다 — 완료 이력을 되돌리지 않는다 | 새 태스크는 평소 승인·검증 흐름 그대로 |
| 요구사항이 완전히 대체됨(부분 수정이 아니라 폐기) | 옛 F-xx는 지우지 않고 "대체됨 → F-yy 참조"로 표시한다(ID 재사용 금지 — 다른 문서·대화가 이미 그 번호를 가리킬 수 있다), 새 F-yy를 만든다 | 새 요구사항만 확인 |

표의 어느 행에도 안 맞는 변경은 임의로 분류하지 말고 사용자에게 어느 쪽에 가까운지 확인한다 (판단은 규칙으로 절).
